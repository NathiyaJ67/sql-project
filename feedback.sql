...
INSERT INTO FEEDBACK VALUES
  ...
  SELECT * FROM Feedback 
  WHERE Submission_Date BETWEEN "2024-01-01" AND "2024-01-15";
  ----------------------------------------------------------------------------------------------------------------------------------------
  
SELECT Name, Category_name, Feedback_text, Rating, Submission_Date
FROM Feedback 
JOIN Customer ON Feedback.Customer_id= Customer.Customer_id
JOIN Category ON Category.Category_id = Feedback.Category_id
ORDER BY Feedback.Submission_Date DESC
LIMIT 5;
----------------------------------------------------------------------------------------------------------------------------------------

SELECT Name, Feedback_id, Feedback_text, Rating 
FROM Feedback 
JOIN Customer ON Feedback.Customer_id = Customer.Customer_id 
WHERE Feedback.rating > 4;
----------------------------------------------------------------------------------------------------------------------------------------

SELECT  Customer.Customer_id, Category.Category_id,feedback_id, feedback_text, Submission_Date
FROM Customer
 JOIN feedback ON Feedback.Customer_id = Customer.Customer_id
JOIN   Category ON Feedback.Category_id =  Category. Category_id
WHERE Feedback.Submission_Date >= DATE_SUB("2024-01-15", INTERVAL 7 DAY);
----------------------------------------------------------------------------------------------------------------------------------------

SELECT Feedback_id, Name, Feedback_text, Rating, Submission_date,Category.Category_id
FROM Feedback 
JOIN Customer ON Feedback .Customer_id = Customer.Customer_id
JOIN Category ON Category.Category_id = Feedback.Category_id
WHERE Category.Category_id = "C01";
----------------------------------------------------------------------------------------------------------------------------------------
...
SELECT
    ...
    Email,
    COUNT(Feedback.Feedback_id) AS TotalFeedback,
    ROUND(AVG(Feedback.Rating), 2) AS AverageRating
FROM
    Customer
